...
def main():
    ...
    #myplot(lista=lista)
    t1=Thread(target=generate_and_subsample, args=(freq_inziale, freq_desiderata))
    t1.start()
    #aspetto il completamento del thread
    t1.join()
    #creo e avvio il thread pe scrivere i dati nel CSV
    t2=Thread(targer=write_to_csv, args=(data_queue.get(), freq_desiderata))
    t2.start()
    t2.join()

    #creo e avvio il thread per fare il plot dei dati
    t3=Thread(target=myplot, args=(data_queue.get(),))
    t3.start()
    t3.join()
